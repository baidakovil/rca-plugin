...
- Add XML doc comments (`///`) above every public class and method following [csharp-docs rules](./csharp-docs.md)
...
- Write unit tests for all non-UI logic in separate test projects
- Use dependency injection for services and providers to enable proper testing
- Follow [async best practices](./csharp-async.md) for async code patterns
...
- Create comprehensive XML documentation comments for all public classes, interfaces, methods, and properties as described in [C# documentation rules](./csharp-docs.md)
...
## Work Output Guidelines

- **Do not create summary/report files** - Unless explicitly requested by the user, do not create README files, summary documents, migration reports, or any other documentation files documenting your work
- Only work with actual project code and files specified by the user
- If documenting work is needed, ask the user for explicit permission first
