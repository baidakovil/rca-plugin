...
0. Generated code will be maintained by AI, so write it with that in mind by implementing these principles:
a) Generate linear, step-by-step code flows 
b) Create comprehensive WHY documentation
c) Explicit error conditions
d) Prefer immutable data structures 
...
4. Keep each method focused on a single responsibility and follow SOLID principles.
...
14. Write unit tests for all non-UI logic in a separate test project. Follow [csharp-nunit.prompt.md](instructions/csharp-nunit.prompt.md) when writing tests.
...
20. All the generated code must compile without errors or warnings. Do not stop generating until it does.
21. Follow [csharp-async.prompt.md](instructions/csharp-async.prompt.md) when writing async code.