...
description: 'Get best practices for C# async programming in Revit add-in context'
---
...
- In Revit context, be aware that `ConfigureAwait(false)` may cause thread switching issues with Revit API
...
- Be careful with `ConfigureAwait(false)` in Revit context - may need UI thread for API calls
