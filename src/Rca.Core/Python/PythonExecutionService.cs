...
namespace Rca.Core.Python
{
    ...
    public class PythonExecutionService
    {
        ...
        /// </summary>
        /// <param name="loggingService">The logging service to use for logging Python execution results.</param>
        public PythonExecutionService(ILoggingService loggingService)
        {
        ...
        private string FormatSuccessAndLog(string output)
        {
            _loggingService.LogPythonOutput(StartMarker);
            _loggingService.LogPythonOutput($"Output: {output}");
            _loggingService.LogPythonOutput(EndMarker);
        ...
        private string FormatErrorAndLog(string errorMessage)
        {
            _loggingService.LogError(ErrorStartMarker);
            _loggingService.LogError($"Python Error: {errorMessage}");
            _loggingService.LogError(ErrorEndMarker);
        ...
        private class ExecutePythonExternalEventHandler : IExternalEventHandler
        {
            ...
            public void Execute(UIApplication app)
            {
                try
                {
                    ...
                    pendingTcs?.TrySetResult(service.FormatSuccessAndLog(output));
                }
                catch (Exception ex)
                {
                    pendingTcs?.TrySetResult(service.FormatErrorAndLog(ex.Message));
                }
                finally
                {
                    // clear state
