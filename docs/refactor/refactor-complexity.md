...
```powershell
dotnet tool run metricsreporter metrics-reader readany --namespace <given_namespace> --metric Complexity
```
...
```powershell
dotnet tool run metricsreporter metrics-reader test --symbol <symbol_been_refactored> --metric Complexity
```
