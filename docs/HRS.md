...
- The source-hash generator tool (`Rca.SourceHashGenerator`, project under `src/Tools/Rca.SourceHashGenerator`) computes a group hash and creates a marker file inside the deploy timestamp folder named `SourceHash-<Group>-<shortHash>.txt` (for example `SourceHash-Runtime-c06c76.txt`).
